...
**Fase: Implementação em andamento, avançando módulo a módulo.** Os dois repositórios já existem e têm código. `bomberquiz-api` (produção em `api.bomberquiz.com.br` via Fly.io + Neon) tem Auth, Perfil, Conteúdo Admin (eixos/matérias/perguntas/fila de revisão), Conteúdo Parceiro, Quiz e Geração de Questões por IA completos — backend e frontend. `bomberquiz-web` (produção em `app.bomberquiz.com.br` via Cloudflare Pages) tem as telas correspondentes a todos esses módulos. Único módulo ainda não implementado: Assinaturas (sem nenhuma linha de código). Ver `docs/tarefas.md` para o estado detalhado por fatia (slice) de cada módulo.
