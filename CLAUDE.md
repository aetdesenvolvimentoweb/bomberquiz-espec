...
**Fase: MVP completo, em validação em produção.** Os dois repositórios já existem e têm código. `bomberquiz-api` (produção em `api.bomberquiz.com.br` via Fly.io + Neon) tem Auth, Perfil, Conteúdo Admin (eixos/matérias/perguntas/fila de revisão), Conteúdo Parceiro, Quiz e Assinaturas completos — backend e frontend. `bomberquiz-web` (produção em `app.bomberquiz.com.br` via Cloudflare Pages) tem as telas correspondentes a todos esses módulos. Geração de Questões por IA (Módulo 7) também foi construída por completo, mas foi **retirada de produção em 2026-07-27** após um incidente de OOM (ver ADR-0039 em `docs/decisoes.md`) — hoje substituída operacionalmente por `ai-bot` (ver parágrafo abaixo). Assinaturas (Módulo 6) foi concluído em 2026-08-28, fechando o ciclo "caminho do cliente pagante" ponta a ponta: trial de 7 dias, gate de acesso do quiz, checkout PIX e cartão (webhook do Mercado Pago validado em produção com pagamento real), lembretes de expiração, e as telas de planos, pagamento, minha assinatura, histórico e reembolso. Seguem **fora do escopo do MVP** por decisão explícita: cortesias, cupons, painel financeiro e admin de planos. Ver `docs/tarefas.md` para o estado detalhado por fatia (slice) de cada módulo.
