...
**Fase: Implementação em andamento, avançando módulo a módulo.** Os dois repositórios já existem e têm código. `bomberquiz-api` (produção em `api.bomberquiz.com.br` via Fly.io + Neon) tem Auth, Perfil, Conteúdo Admin (eixos/matérias/perguntas/fila de revisão), Conteúdo Parceiro e Quiz completos — backend e frontend. `bomberquiz-web` (produção em `app.bomberquiz.com.br` via Cloudflare Pages) tem as telas correspondentes a todos esses módulos. Geração de Questões por IA (Módulo 7) também foi construída por completo, mas foi **retirada de produção em 2026-07-27** após um incidente de OOM (ver ADR-0039 em `docs/decisoes.md`) — hoje substituída operacionalmente por `ai-bot` (ver parágrafo abaixo). Assinaturas (Módulo 6) é o único módulo **ainda incompleto**: backend de cobrança PIX (checkout, webhook do Mercado Pago validado em produção com pagamento real), trial de 7 dias e gate de acesso do quiz já existem; faltam cartão, cortesias, cupons, reembolso, lembretes, painel financeiro e **todo o frontend**. Ver `docs/tarefas.md` para o estado detalhado por fatia (slice) de cada módulo.
