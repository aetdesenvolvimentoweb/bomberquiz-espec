...
## 0022 — Modelo LLM para geração de questões: Claude (Anthropic) (2026-06-17, revisado 2026-07-24)
...
**Decisão:** **Claude** (modelo `claude-sonnet-5` como padrão — revisado de `claude-sonnet-4-6` na implementação de 2026-07-24: Sonnet 5 já estava disponível, com preço promocional e qualidade superior ao 4.6, sem motivo para fixar o modelo anterior), via API da Anthropic. Justificativas: (a) janela de contexto ampla — suficiente para manuais completos sem chunking na maioria dos casos; (b) qualidade superior em português técnico e em seguir instruções de formato estruturado (tool use estrito); (c) consistência no domínio de educação/concurso; (d) o stack Claude Code já pressupõe familiaridade com a API Anthropic. Sonnet é o default pelo equilíbrio custo/qualidade; `claude-opus-4-8` é alternativa para casos que exijam mais elaboração (AIGEN-P-04). O modelo fica em env (`ANTHROPIC_MODEL_AI_GENERATION`), não hardcoded — trocar é mudança de config.
**Consequências:** Nova dependência externa: `ANTHROPIC_API_KEY` em `.env`. Custo estimado de R$ 0,50–2,00 por job de 20 questões (varia com o modelo/preço vigente). Rate limiting de jobs (10/admin/dia, 2 simultâneos globais) controla gasto. Adicionar a chave ao checklist de bootstrap do `bomberquiz-api` e ao runbook de ambientes. Geração de JSON estruturado via tool use forçado (`tool_choice` apontando para uma tool `generate_questions` com `strict: true`), não `output_config.format` — mais portável entre modelos.
...
## 0035 — Scheduler in-process do Módulo 5: biblioteca `croner` (2026-07-22)

**Contexto:** ADR-0017 já tinha decidido o desenho geral (scheduler in-process, um job = um caso de uso em `application/`, disparado por `infra/scheduler/`, fuso `America/Sao_Paulo`, idempotente, horários em env) mas não fixava a biblioteca. A Slice 2 do Módulo 5 (QUIZ-RF-004 — expiração/auto-abandono de quiz a cada 5 min) é o primeiro consumidor real dessa infra, hoje vazia (`infra/scheduler/` só tem `.gitkeep`). Opções avaliadas: `setInterval` manual (zero dependência nova, mas não lida bem com horário fixo/DST para os jobs diários futuros — recálculo de dificuldade às 00:00 de CONT-RF-017, lembretes de assinatura às 09:00 de SUB-RF-007); `node-cron` (sem suporte robusto de timezone nativo); `croner` (zero-dependency, suporte nativo a timezone via `Intl`, funciona bem no Bun).
**Decisão:** `croner`. Ainda que o job da Slice 2 precise só de um intervalo fixo de 5 min (que `setInterval` resolveria sozinho), o scheduler é desenhado para ser reaproveitado pelos jobs diários futuros (CONT-RF-017, SUB-RF-007, AUTH-RF-008, dump de backup) — mais barato introduzir a primitiva certa agora do que trocar depois. `infra/scheduler/scheduler.ts` (wrapper fino: `registerJob`/`startScheduler`) captura e loga erro por job sem derrubar o processo nem travar os demais; horários vêm de env (`QUIZ_EXPIRY_JOB_CRON`, default `*/5 * * * *`), não hardcoded.
**Consequências:** Primeira dependência de runtime nova desde o bootstrap do backend. Mantém a premissa de instância única já assumida pelo rate-limit in-memory (`fly.toml` com `min_machines_running=1`) — migração para `pg_advisory_lock`/Fly scheduled machines fica documentada como caminho futuro se a premissa mudar (mesma nota de ADR-0017).

## 0036 — Biblioteca de gráficos do frontend: `recharts` (2026-07-22)

**Contexto:** QUIZ-RF-007 (painel de desempenho) e QUIZ-RF-010 (evolução temporal) exigem visualização de dados (barras de acurácia por matéria/eixo, gráfico de linha mensal) — nenhuma lib de chart está instalada no `bomberquiz-web` até aqui (nenhum módulo anterior precisou). Avaliado contra `chart.js` (imperativo, menos idiomático em React) e `visx` (mais baixo nível, mais código pra montar os mesmos gráficos simples).
**Decisão:** `recharts` — composable e declarativo no estilo React, integra bem com Tailwind, bundle razoável para um PWA mobile-first. Usado nas Slices 5-6 (frontend) do Módulo 5, ainda não implementadas nesta rodada.
**Consequências:** Segunda dependência de runtime nova do ciclo (após `croner`, ADR-0035). Nenhum gráfico existe em produção ainda — primeira vez que o design system do projeto precisa incorporar visualização de dados; se os tokens de cor de ADR-0032 não cobrirem bem uma paleta categórica para múltiplas matérias/eixos, revisitar ali antes de a Slice 6 chegar na implementação real.

**Nota (2026-07-24, Slice 6 do Módulo 5):** A pendência acima não se concretizou. O painel de desempenho colore barras por `status_badge` (ordinal — `unrated`/`fraco`/`medio`/`forte`), não por identidade de matéria/eixo, e a timeline é uma série única (accuracy ao longo do tempo) — nenhum dos dois gráficos precisa de paleta categórica. Os tokens semânticos já existentes (`--muted-foreground`, `--destructive`, `--gold`, `--success`, ADR-0032) cobrem os quatro níveis do `status_badge` sem criar nada novo. ADR-0032 não precisou ser revisitada.
