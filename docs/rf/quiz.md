...
**Descrição:**
`POST /quizzes` e `POST /quizzes/:id/answers` só funcionam quando o cliente está em **período gratuito ativo** **ou** com **assinatura vigente** (paga ou cortesia). Bloqueio retorna HTTP 402 sinalizando a UI a oferecer planos. (A Descrição citava `GET /me/performance` como bloqueado, contradizendo o CA-3 abaixo; corrigido na implementação — desempenho é leitura e permanece liberado.)

> ✅ **Status de implementação (2026-08-26):** implementado junto da fatia 1 do Módulo 6. Middleware `requireActiveAccess` (`api/src/http/middleware/access.middleware.ts`) consome a porta `IAccessPolicy` (SUB-RF-011) e está registrado apenas nas duas rotas exigidas pelo CA-4. `POST /quizzes/:id/finish` **não** é bloqueado: quem perde o acesso no meio de um quiz ainda consegue encerrá-lo e ver o resultado.

**Critérios de aceitação:**
- **CA-1:** Middleware avalia, a cada request a endpoint de quiz, se `user.access_status` é `active` (valor calculado pelo Módulo 6, consolidando trial + subscriptions). Se não, retorna HTTP 402. **Aditivo de implementação (2026-08-26):** o corpo segue o **envelope de erro padrão** do projeto (`{ error: { code: "subscription_required", message, request_id } }`), não o payload ad-hoc `{ reason, trial_used, last_active_until }` desta spec — consistência com todos os outros erros da API vale mais que o campo extra, e `trial_used`/`active_until` já vêm de `GET /me/subscription` (SUB-RF-005), que é para onde a UI leva o cliente ao receber o 402.
